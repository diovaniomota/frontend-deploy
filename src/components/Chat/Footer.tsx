...
export const Footer = ({
    ...
    return (
        <>
            <div className={`fixed ml-2 ${emoijPicker ? "opacity-100 bottom-16" : "-bottom-[440px] opacity-0"} duration-300`}>
                <Picker
                    data={data}
                    theme={theme.theme}
                ...
                {isRecording ?
                    <div className="flex items-center gap-5">
                        <Button variant='ghost' size='icon' title="Parar Gravação" onClick={handleDeleteRecording}>
                            <Trash className="text-slate-500 dark:text-slate-300 h-5 w-5" />
                        </Button>
