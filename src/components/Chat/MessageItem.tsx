                    ...
                    data.content_type.startsWith('video/') ?
                        ...
                        <div className="flex items-center gap-3.5 py-1 px-2.5">
                            ...
                            <div>
                                ...
                                <p className="text-sm">{data.size} • {data.content_type}</p>
                            </div>
                        </div>
                }
            </a>
        ...
        <div className="flex gap-3 items-center">
            {data.from_user.id == user?.id &&
                <DropdownMenu>
                    <DropdownMenuTrigger>
                        <EllipsisVertical
                            className="size-5 text-slate-500 dakr:text-slate-400 hover:text-primary cursor-pointer -mt-5"
                        />
                    </DropdownMenuTrigger>
            ...
            <div className="space-y-1">
                <div
                    className={`${data.from_user.id === user?.id ? 'bg-primary rounded-l-md rounded-ee-md text-primary-foreground' : 'bg-secondary rounded-r-md rounded-es-md text-slate-700 dark:text-slate-200'} max-w-xs sm:max-w-md py-2 px-3`}
                >
                ...
                <div className="flex items-center justify-between px-1 gap-6 select-none">
                    <small className="text-slate-600 dark:text-slate-400 font-semibold">{dayjs(data.created_at).format('DD/MM/YYYY [às] HH:mm')}</small>
